...
router.post("/", validate(multiChatMessageSchema), async (req, res) => {
  ...
  try {
    // Query chunks directly. If chunks exist, the PDF has already been processed,
    // regardless of whether it was uploaded before or after BullMQ.
    const filter = { owner: req.user.id };

    if (effectiveScope !== ALL_SCOPE) {
      filter.subject = effectiveScope;
    }
    ...
    if (chunkCount === 0) {
      ...
      const chunks = await Chunk.find(filter)
        .select("text page subject embedding")
        .lean();
      ...
      if (bestScore(topChunks) < SIMILARITY_THRESHOLD) {
        ...
        fullAnswer = await streamAnswerAcrossNotes(
          message,
          topChunks,
          sendToken,
          controller.signal
    ...
    if (!res.headersSent) {
      res
        .status(500)
        .json({ error: "Couldn't generate an answer right now. Please try again." });
    } else {
      res.write(
        `data: ${JSON.stringify({
