...
export function explainNextBestAction({
  ...
  const imp = clamp01(examImportance);
  const urg = clamp01(examUrgency);

  if (prerequisiteBlocked) {
    if (prerequisiteName) {
      reasons.push(`Prerequisite "${prerequisiteName}" needs attention before this concept.`);
    } else {
  ...
  if (imp >= 0.7 || urg >= 0.7) {
    reasons.push("This concept has high importance for your upcoming exam.");
  }
