import { createContext, useContext, useEffect, useState } from "react";
import client, { TOKEN_KEY, REFRESH_TOKEN_KEY } from "./client.js";

const AuthContext = createContext(null);

export function AuthProvider({ children }) {
  const [email, setEmail] = useState(null);
  const [ready, setReady] = useState(false);

  useEffect(() => {
    const token = localStorage.getItem(TOKEN_KEY);
    if (!token) {
      setReady(true);
      return;
    }
    client
      .get("/auth/me")
      .then(({ data }) => setEmail(data.email))
      .catch(() => {
        localStorage.removeItem(TOKEN_KEY);
        localStorage.removeItem(REFRESH_TOKEN_KEY);
      })
      .finally(() => setReady(true));
  }, []);

  async function login(loginEmail, password) {
    const { data } = await client.post("/auth/login", { email: loginEmail, password });
    localStorage.setItem(TOKEN_KEY, data.accessToken);
    if (data.refreshToken) localStorage.setItem(REFRESH_TOKEN_KEY, data.refreshToken);
    setEmail(data.email);
  }

  async function register(regEmail, password) {
    const { data } = await client.post("/auth/register", { email: regEmail, password });
    localStorage.setItem(TOKEN_KEY, data.accessToken ?? data.token);
    if (data.refreshToken) localStorage.setItem(REFRESH_TOKEN_KEY, data.refreshToken);
    setEmail(data.email);
  }

  function logout() {
    localStorage.removeItem(TOKEN_KEY);
    localStorage.removeItem(REFRESH_TOKEN_KEY);
    setEmail(null);
  }

  async function forgotPassword(forgotEmail) {
    const { data } = await client.post("/auth/forgot-password", { email: forgotEmail });
    return data;
  }

  async function resetPassword(resetEmail, token, newPassword) {
    const { data } = await client.post("/auth/reset-password", { email: resetEmail, token, newPassword });
    return data;
  }

  return (
    <AuthContext.Provider value={{ email, ready, login, register, logout, forgotPassword, resetPassword }}>
      {children}
    </AuthContext.Provider>
  );
}

export function useAuth() {
  return useContext(AuthContext);
}